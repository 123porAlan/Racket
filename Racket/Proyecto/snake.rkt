...
(define (all-but-last segs)
  (cond
    [(= (length segs) 1) empty]
    [else (cons (car segs) (all-but-last (cdr segs)))])
 #| Aquí va su código. |# )
...
(define (opposite-dir? d1 d2)
  (cond
    [(and (equal? d1 "up") (equal? d2 "down")) #true]
    [(and (equal? d2 "up") (equal? d1 "down")) #true]
    [(and (equal? d1 "right") (equal? d2 "left")) #true]
    [(and (equal? d2 "right") (equal? d1 "left")) #true]
    [else #false])
  #| Aquí va su código |#)
